...
def main() -> int:
    ...
    try:
        ...
        # returned, dropping the last reference ran the C++ destructor, and on
        # Windows that can sit forever after a suite has already passed -- the
        # verify gate then waits on a process with nothing left to do. The
        # module-level ``os._exit`` below is the real end of this process.
